...
class ETM(nn.Module):
    ...
    def forward(self, bow_dict, theta=None, aggregate=True):
        ...
        recon_loss = -(preds * bow_dict['bow'].to(self.device)).sum(1)
        if aggregate:
    ...
    @classmethod
    def generate_embeddings(cls, vocab, vocab_size, device, save_path):
        """
        ...
        """
        vectors = {}
        with open(settings.emb_path, 'rb') as f:
            for l in f:
                line = l.decode().split()
                word = line[0]
                if word in vocab:
                    ...
                    vectors[word] = vect
        embeddings = np.zeros((vocab_size, settings.emb_size))
        words_found = 0
        for i, word in enumerate(vocab):
            try: 
                ...
                embeddings[i] = np.random.normal(scale=0.6, size=(settings.emb_size, ))
...
class GetWeightedTopics(nn.Module):
    ...
    def __init__(self, num_topics):
        ...
        self.num_topics = num_topics
#         self.W = nn.parameter.Parameter(torch.Tensor(torch.randn(num_topics) + 1.))
...
class TripletNet(pl.LightningModule):
    def __init__(self, embeddings, vocab_size, device, lr=1, lr_w=1, weight_decay=0, margin=1, freeze_encoder=True):
        """
        Args:
            embeddings(torch.tensor, optional): pretrained topic embeddings   
            vocab_size (int): size of the corpus vocabulary
        ...
        self.t_hidden_size = settings.t_hidden_size
        self.rho_size = settings.rho_size
        self.emb_size = settings.emb_size
        self.theta_act = settings.theta_act
        self.enc_drop = settings.enc_drop
        ...
        if freeze_encoder:
            for par in self.etm.parameters():
